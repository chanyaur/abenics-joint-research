...
%
% Model interface (code convention; see params_pid_plant.m for the naming
...
%
% Run params_pid_plant first (this script also wires it as the model PreLoadFcn
% so 'pp' resolves from the base workspace whenever the model is opened/run).
    ...
    set_param(mdl, 'PreLoadFcn', 'params_pid_plant;');
...
function buildPID(sub)
    ...
    set_param(pid, ...
        ...
        'AntiWindupMode',        'clamping');         % ... with anti-windup
